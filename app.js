...
const RobotScene = (() => {
  ...
  let cameraRenderers = [], onHandCamera, globalViewCamera;
  ...
  function init(container) {
    ...
    // 二次 WebGL 渲染器 (Camera 面板) — 桌面版 & 手機版（直向 Tab）都要各自建立一個
    function setupCameraRenderer(canvasId) {
      const el = document.getElementById(canvasId);
      if (!el) return null;
      const r = new THREE.WebGLRenderer({ canvas: el, antialias: true });
      r.setPixelRatio(Math.min(window.devicePixelRatio, 2));
      const rect = el.parentElement.getBoundingClientRect();
      r.setSize(rect.width || 1, rect.height || 1);
      return { renderer: r, canvasEl: el };
    }

    cameraRenderers = [];
    ["cameraCanvas", "cameraCanvasMobile"].forEach(id => {
      const entry = setupCameraRenderer(id);
      if (entry) cameraRenderers.push(entry);
    });
    ...
    window.addEventListener("resize", () => {
      ...
      cameraRenderers.forEach(({ renderer: r, canvasEl }) => {
  ...
  function updateFrame(dtMs) {
    ...
    // 桌面版與手機版（直向 Tab）畫布都各自渲染一次；
    // 若畫布當下不可見（如尚未切到該分頁）則寬高為 0，直接跳過避免報錯
    if (cameraRenderers.length && typeof CameraSimulator !== 'undefined') {
      ...
      if (activeCam) {
        cameraRenderers.forEach(({ renderer: r, canvasEl }) => {
          ...
          r.getSize(size);
          if (Math.round(size.x) !== Math.round(rect.width) || Math.round(size.y) !== Math.round(rect.height)) {
            r.setSize(rect.width, rect.height);
          }

          r.render(scene, activeCam);
        });
      }
    }
  }
...
function sparkline(canvas, values, color, min, max) {
  if (!canvas) return;   // null-guard：canvas 不存在時直接忽略
  const ctx = canvas.getContext("2d");
  const w = canvas.clientWidth, h = canvas.clientHeight;
  if (!w || !h) return;  // 尚未佈局完成時跳過
...
// 建立事件項目 DOM，同時推入橫向 & 直向（手機）兩個 Event List
function addEventItem({ kind, title, desc, time }) {
  ...
  // 橫向 Event Log（Overlay 版）
  eventListEl.prepend(makeEl());
  while (eventListEl.children.length > 40) eventListEl.removeChild(eventListEl.lastChild);
  applyEventFilter();

  // 直向 Event Log（手機 Tab 版）
  const mobileListEl = document.getElementById("eventListMobile");
  if (mobileListEl) {
    mobileListEl.prepend(makeEl());
    while (mobileListEl.children.length > 40) mobileListEl.removeChild(mobileListEl.lastChild);
    applyMobileEventFilter();
  }
}
...
function applyMobileEventFilter() {
  const mobileListEl = document.getElementById("eventListMobile");
  const filterEl = document.getElementById("eventFilterMobile");
  if (!mobileListEl || !filterEl) return;
  const f = filterEl.value;
  [...mobileListEl.children].forEach(el => {
    el.style.display = (f === "all" || el.dataset.kind === f) ? "" : "none";
  });
}
const mobileFilterEl = document.getElementById("eventFilterMobile");
if (mobileFilterEl) mobileFilterEl.addEventListener("change", applyMobileEventFilter);
...
// 同時更新橫向桌面版 (chartXxx) 與直向手機版 (chartXxxM) 的 Canvas
function updateTrends(state) {
  ...
  // 手機版數值文字（存在時才更新）
  const sM = document.getElementById("speedValM"); if (sM) sM.textContent = speed.toFixed(1);
  const aM = document.getElementById("accelValM"); if (aM) aM.textContent = accel.toFixed(1);
  const vM = document.getElementById("vibValM"); if (vM) vM.textContent = vibration.toFixed(3);

  // 桌面 Sparklines
  sparkline(document.getElementById("chartSpeed"), history.speed, "rgb(91,143,204)", 0, 600);
  sparkline(document.getElementById("chartAccel"), history.accel, "rgb(74,158,110)", 0, 300);
  sparkline(document.getElementById("chartVib"), history.vib, "rgb(181,129,58)", 0, 0.1);
  // 手機 Sparklines
  sparkline(document.getElementById("chartSpeedM"), history.speed, "rgb(91,143,204)", 0, 600);
  sparkline(document.getElementById("chartAccelM"), history.accel, "rgb(74,158,110)", 0, 300);
  sparkline(document.getElementById("chartVibM"), history.vib, "rgb(181,129,58)", 0, 0.1);

  // 馬達溫度與電流雙曲線混合圖 Canvas 渲染（桌面 & 手機）
  function renderMotor(canvasId) {
    ...
    if (canvas.height !== h) canvas.height = h;
    ctx.clearRect(0, 0, w, h);
    drawLine(ctx, history.temp, w, h, "rgb(181,129,58)", 0, 60);
    drawLine(ctx, history.curr, w, h, "rgb(74,158,110)", 0, 4);
  }
  renderMotor("chartMotor");
  renderMotor("chartMotorM");
}
...
const CameraSimulator = (() => {
  let hudCanvases = []; // [{canvas, ctx}] 桌面版 + 手機版
  let cameraMode = "onhand";
  let robotState = null;
  let t = 0;
  let selectEls = []; // 桌面版 + 手機版下拉選單

  function init() {
    hudCanvases = [];
    ["cameraHudCanvas", "cameraHudCanvasMobile"].forEach(id => {
      const canvas = document.getElementById(id);
      if (canvas) hudCanvases.push({ canvas, ctx: canvas.getContext("2d") });
    });

    selectEls = [];
    ["cameraSelect", "cameraSelectMobile"].forEach(id => {
      const sel = document.getElementById(id);
      if (sel) {
        selectEls.push(sel);
        sel.addEventListener("change", (e) => setCameraMode(e.target.value));
      }
    });
    if (selectEls[0]) cameraMode = selectEls[0].value || "onhand";
  ...
  function setCameraMode(mode) {
    cameraMode = mode;
    // 保持桌面版與手機版下拉選單狀態同步
    selectEls.forEach(sel => { if (sel.value !== mode) sel.value = mode; });
    updateLabel();
  }
  ...
  // HUD 繪製主流程 — 桌面版與手機版畫布都各自繪製一次
  function drawFrame(state) {
    robotState = state;
    t += 16;
    if (!hudCanvases.length) return;
    resizeCanvases();

    hudCanvases.forEach(({ canvas, ctx }) => {
      const w = canvas.width, h = canvas.height;
      if (!w || !h) return; // 尚未顯示（如手機分頁未切換到）時跳過

      // 清除 2D 畫布，使下方 WebGL 3D 畫面透出
      ctx.clearRect(0, 0, w, h);

      if (cameraMode === "global") {
        drawGlobalHud(ctx, w, h);
      } else {
        drawOnHandHud(ctx, w, h);
      }
  ...
  // 全景 HUD 繪製
  function drawGlobalHud(ctx, w, h) {
  ...
  // FPV 第一人稱 HUD 繪製
  function drawOnHandHud(ctx, w, h) {
    // const conveyorState = robotState?.conveyor?.state || robotState?.robot?.state || "STOP";
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // ── 直向（Portrait）Tab 切換邏輯 ──────────────────────────────────
  // 偵測所有 .mobile-tab-btn，點擊時切換對應 .mobile-tab-pane 的 active 狀態
  document.querySelectorAll(".mobile-tab-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const target = document.getElementById(`mobile-pane-${pane}`);
      if (target) target.classList.add("active");
    });
  });

  // ── 3D Viewport 大小：隨視窗 resize 保持正方形 ──────────────────
  // Three.js Renderer resize 已由 RobotScene 內部的 ResizeObserver 處理
  // 此處強制在 resize 後重算 viewport-panel 的 CSS 尺寸以保持正方形
  function syncViewportSize() {
    const vp = document.getElementById("viewport-panel");
    if (!vp) return;
    const isPortrait = window.matchMedia("(orientation: portrait)").matches;
    if (isPortrait) {
      // 直向：邊長 = 視窗寬度（已由 CSS aspect-ratio + width:100% 處理）
      vp.style.width = "";
      vp.style.height = "auto";
    } else {
      // 橫向：邊長 = 主網格可用高度（parent flex height）
      vp.style.width = "";
      vp.style.height = "";
    }
  }
